...
def answer_is_correct(operation, random_number1, random_number2, user_answer):
    # Определяем тип операции и правильный ответ
    if operation == OPERATION_PLUS:
        math_result = random_number1 + random_number2
...
def generate_game_round(name):
    ...
    while user_answer is True and game_round <= GAME_ATTEMPTS:
        ...
        computer_question = '{} {} {}'.format(random_number1, operation, random_number2)
        print('Question: {}'.format(str(computer_question)))
        user_answer = prompt.integer('Your answer: ')

        # Определяем тип операции и правильный ответ
        bool_answer_result, math_result = answer_is_correct(operation, random_number1, random_number2, user_answer)
